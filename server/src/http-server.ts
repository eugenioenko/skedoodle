...
const HTTP_PORT = process.env.HTTP_PORT || 3013;
const WS_PORT = process.env.WS_PORT || 3014; // Use a different port for WS to avoid conflict

app.use(express.json());
app.use(cors({
    origin: 'http://localhost:5173', // Allow client to connect
    credentials: true,
}));

// Make Prisma client and verifyToken available to routes
app.use((req, res, next) => {
    (req as any).prisma = prisma;
    (req as any).verifyToken = verifyToken; // Make verifyToken available
    next();
});
