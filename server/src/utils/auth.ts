...
export async function verifyOidcToken(token: string): Promise<{ userId: string; username: string }> {
...
username = (userinfo.preferred_username ?? userinfo.name ?? userinfo.email ?? oidcSub) as string;
        } else {
